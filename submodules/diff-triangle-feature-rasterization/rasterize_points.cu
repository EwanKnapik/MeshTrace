	...
	const bool include_feature)
{
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		instance_feature.contiguous().data_ptr<float>(),
		...
		out_instance_feature.contiguous().data_ptr<float>(),
  ...
  return std::make_tuple(rendered, out_color, out_instance_feature, out_others, radii, was_rendered, geomBuffer, binningBuffer, imgBuffer, scaling, max_blending);
}
	...
	const bool include_feature) 
{
  ...
  if (include_feature) {
	dL_dinstance_feature = torch::zeros({V, NUM_CHANNELS_instance_feature}, vertices.options());
  } else {
	dL_dinstance_feature = torch::zeros({1}, vertices.options());
  }
  ...
  {  
	  ...
	  dL_dout_instance_feature.contiguous().data_ptr<float>(),
	  ...
	  dL_dinstance_feature.contiguous().data_ptr<float>(),
  ...
  return std::make_tuple(dL_dvertices3D, dL_dvertice_weight, dL_dcolors, dL_dinstance_feature, dL_dsh);
}
