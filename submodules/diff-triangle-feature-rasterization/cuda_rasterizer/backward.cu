		 ...
		 for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		 {
			 ...
			 if (include_feature) {
				for (int ch = 0; ch < F; ++ch) {
					float f0 = instance_feature[vertex_idx0 * F + ch];
					float f1 = instance_feature[vertex_idx1 * F + ch];
					float f2 = instance_feature[vertex_idx2 * F + ch];
					float interp_feature = wA * f0 + wB * f1 + wC * f2;

					accum_rec_F[ch] = last_alpha * last_instance_feature[ch] + (1.f - last_alpha) * accum_rec_F[ch];
					last_instance_feature[ch] = interp_feature;

					const float dL_dchannel_F = dL_dpixel_F[ch];
					dL_dalpha += (interp_feature - accum_rec_F[ch]) * dL_dchannel_F;

					float dL_dfeature_ch = dchannel_dcolor * dL_dchannel_F;
					sum0 += dL_dfeature_ch * f0;
					sum1 += dL_dfeature_ch * f1;
					sum2 += dL_dfeature_ch * f2;

					atomicAdd(&dL_dinstance_feature[vertex_idx0 * F + ch], dL_dfeature_ch * wA);
					atomicAdd(&dL_dinstance_feature[vertex_idx1 * F + ch], dL_dfeature_ch * wB);
					atomicAdd(&dL_dinstance_feature[vertex_idx2 * F + ch], dL_dfeature_ch * wC);
				}
			 }
