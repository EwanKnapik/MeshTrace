		 ...
		 for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		 {
			 ...
			 if (include_feature) {
				for (int ch = 0; ch < CHANNELS_instance_feature; ++ch) {
					float f0 = instance_feature[vertex_idx0 * CHANNELS_instance_feature + ch];
					float f1 = instance_feature[vertex_idx1 * CHANNELS_instance_feature + ch];
					float f2 = instance_feature[vertex_idx2 * CHANNELS_instance_feature + ch];
					float interp = wA * f0 + wB * f1 + wC * f2;
					F[ch] += interp * alpha * T;
				}
			 }
