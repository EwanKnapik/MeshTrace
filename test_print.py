...
def print_img_from_id_map(viewpoint_camera, pc :TriangleModel, pipe, bg_color) -> torch.tensor:
    ...
    plt.axis("off")
    plt.savefig(f"instance_map/instance_map_{viewpoint_camera.colmap_id}.png", bbox_inches="tight", pad_inches=0)
    sixel_fig()
    plt.close()

    return

def print_img_from_feature_rasterizer(viewpoint_camera, pc :TriangleModel, pipe, bg_color, scene_name) -> torch.tensor:
    ...
    plt.axis("off")
    #sixel_fig()
    plt.savefig(f"instance_map/instance_map_{scene_name}_{viewpoint_camera.colmap_id}.png", bbox_inches="tight", pad_inches=0)
    plt.close()
...
def main():
    ...
    print_img_from_feature_rasterizer(viewpoint_cam,triangles,pipe,background,os.path.basename(dataset.source_path))
